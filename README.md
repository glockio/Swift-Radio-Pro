...
- ```pod install && cd ReactComponents && npm install && cd ..```
- Start react native dev server ```cd ReactComponents && run npm start-dev```
- Release
	``` $ react-native bundle --dev false --platform ios --entry-file index.ios.js --bundle-output ../main.jsbundle --minify ```


- Push
	code-push release-react SwiftRadioRN ios --entryFile index.ios.js -d Staging --targetBinaryVersion "*"
#Swift Radio
